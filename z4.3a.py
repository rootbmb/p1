...
а затем соответствующий шаблон, в который
подставлен номер VLANа (или список VLANов).
Ограничение: Все задания надо выполнять используя только пройденные темы.
То есть эту задачу можно решить
 без использования условия if и циклов for/while.
...
conf_dict = {'access': access_template, 'trunk': trunk_template,
             'vlanaccess': 'Enter VLAN number:',
             'vlantrunk': 'Enter allowed VLANs:'}


mode = input('Enter interface mode (access/trunk):')
interface = input('Enter interface type and number:')
key_vlan = 'vlan' + mode
vlan = input(('{}').format(conf_dict[key_vlan]))


print('\n' + '-' * 30)
print(('interface {}').format(interface))
print(('\n'.join(conf_dict[mode])).format(vlan))
print('-' * 30)
